...
class VMImpl final : public VM {
 public:
  ...
  DynamicComputation InvokeDynamic(absl::AnyInvocable<void()> fn) final {
    ...
    //
    // Note always doing two context switches is not always free, maybe we just
    // have machinery for handling this in stop? You'd have to track this with
    // an extra bit of memory, so unsure what the right tradeoff is.
    _thread->Resume();
    return comp;
  }
